...
#####################################################
# This script runs diamond-type measurement in reflectometry only; automatically saves the matrix file as well

#25.8.2017. It reads also the phase!
#####################################################
...
file_name = ' By=2T gvsg 18-13 f=170MHz VAR15 -15V -30dBm sensor jumps Vsd=100us'
...
v1_vec = arange(-450,101,1)     #V_g
v2_vec = arange(200,600,2)  #V_sd 
...
data_phase = qt.Data(name=file_name + ' phase')
...
data_phase.add_coordinate('V_G (sensor) [mV]')
data_phase.add_coordinate('V_G (dot) [mV]')
data_phase.add_value('Phase')
...
new_mat_phase = np.zeros((len(v2_vec), len(v1_vec)))
...
plot3d = qt.Plot3D(data, name='gatevsgate_sjumps27', coorddims=(1,0), valdim=2, style='image') #flipped coordims that it plots correctly
...
try:
    for i,v1 in enumerate(v1_vec):  # CHANGE THIS LINE FOR MATRIX FILE SAVING
        ...
        for j,v2 in enumerate(v2_vec):  # CHANGE THIS LINE FOR MATRIX FILE SAVING
            ...
            # Save to the matrix
            new_mat[j,i] = result_reflectometry[0]   # ADD THIS LINE FOR MATRIX FILE SAVING
            new_mat_phase[j,i] = result_reflectometry[1]   # ADD THIS LINE FOR MATRIX FILE SAVING

            data.add_data_point(v2, v1, result_reflectometry[0])
            data_phase.add_data_point(v2, v1, result_reflectometry[1]) 
            qt.msleep(0.001)
