...
t_sync = 5              # Duration of synchronization element in sequence in "TimeUnits"
...
if not(Automatic_sequence_generation):  # If user wants manual sequence generation (Automatic_sequence_generation = False)
    ...
    A1 = np.array([0.0,500]) # Initial amplitudes, 31.25 is factor of attenuation
    #delta_A1 = -3.0*31.25/(Seq_length/1.0) 
    #delta_A1 = (1+1.0/Seq_length)*delta_A1
    #A2 = np.array([0.0,3.0,3.0,0.0,0.0])*31.25*0.81 # Initial amplitudes, 31.25 is factor of attenuation, 1.2 is factor for diagonal pulsing
    ...
    for i in xrange(Seq_length):   # Creating waveforms for all sequence elements
        ...
        #p.setValuesCH2([0.005, A1[0]],[0.005, A1[1]],[0.005, A1[2]]) # Setting waveform shape for one wavefrom object p in sequence seq for AWG channel 1 - [Time1,Amp1],[Time2,Amp2]...  Time in TimeUnits and Amp in AmpUnits
