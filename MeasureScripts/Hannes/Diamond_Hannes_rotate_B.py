...
file_name = '15_17 IV 733'

gain = 1000e6 #Choose between: 1e6 for 1M, 10e6 for 10M, 100e6 for 100M and 1e9 for 1G

# you define two vectors of what you want to sweep. In this case
# a magnetic field and a voltage (detuning)


# sweep voltages (detuning)

v1_vec = arange(-840,-829.99,0.05)   #V_g 1

incl = -2  # inclination of sweeping direction

# enter starting point of v2_vec (2 x)
v2_vec = np.linspace(60,60+incl*abs(v1_vec[-1]-v1_vec[0]),len(v1_vec))  #V_g 2



#magnet, choose y or z direction or both

ramp_rate_Z = 0.001 #T/s
ramp_rate_Y = 0.001 #T/s


step_size_alpha = 3.1415/180
 
# define angle of B vector

alpha_vector = arange(0,3.1415+step_size_alpha,step_size_alpha)
...
data.add_coordinate('detuning [mV]')
...
new_mat = np.zeros((len(v1_vec), len(alpha_vector))) # Creating empty matrix for storing all data   - ADD THIS LINE FOR MATRIX FILE SAVING, PUT APPROPRIATE VECTOR NAMES
...
try:
    for i,v3 in enumerate(aplha_vector):  # CHANGE THIS LINE FOR MATRIX FILE SAVING
        
        
        start = time()

        # set B field 
        magnetZ.set_field(0.5*cos((alpha_vector[i]))
        magnetY.set_field(0.5*sin((alpha_vector[i]))  

        #total_field = np.sqrt(BZ_vector[i]**2 + BY_vector[i]**2)

        while math.fabs(0.5*cos(alpha_vector[i]) - magnetZ.get_field_get()) > 0.0001:
            qt.msleep(0.050)
        ...
        for j,v1 in enumerate(v1_vec):  # CHANGE THIS LINE FOR MATRIX FILE SAVING
            ...
            #IVVI.set_dac2(v1)
            #IVVI.set_dac3(v1) 
            IVVI.set_dac3(v2_vec[j])
            IVVI.set_dac2(v1_vec[j])
            #IVVI.set_dac6(v1)
