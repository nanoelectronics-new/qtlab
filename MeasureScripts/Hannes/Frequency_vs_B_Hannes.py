...
#magnetZ = qt.instruments.create('magnetZ', 'AMI430_Bz', address='10.21.64.189')
#magnetY = qt.instruments.create('magnetY', 'AMI430_By', address='10.21.64.144')
#VSG = qt.instruments.create('VSG','RS_SMW200A',address = 'TCPIP::10.21.64.167::hislip0::INSTR')
#dmm_lockin = qt.instruments.create('dmm_lockin','a34410a', address = 'USB0::0x2A8D::0x0101::MY54502777::INSTR')
...
step_size_BZ = -2e-3 
#step_size_BY = 2e-3 
BZ_vector = arange(800e-3,0e-3+step_size_BZ,step_size_BZ) #T  # Those two vectors need to be the same left
#BY_vector = arange(0e-3,500e-3+step_size_BY,step_size_BY) #T  #
...
freq_vec = arange(4e9,10e9,1e6)  # frequency 
...
new_mat = np.zeros((len(freq_vec), len(BZ_vector)),dtype=float16) # Creating empty matrix for storing all data   - ADD THIS LINE FOR MATRIX FILE SAVING, PUT APPROPRIATE VECTOR NAMES
