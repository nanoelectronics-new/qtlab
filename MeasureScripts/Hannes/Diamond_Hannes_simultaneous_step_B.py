...
v1_vec = arange(-348.8,-348.19,0.05)   #V_g 1

incl = -5  # inclination of sweeping direction

# enter starting point of v2_vec (2 x)
v2_vec = np.linspace(-372,-372+incl*abs(v1_vec[-1]-v1_vec[0]),len(v1_vec))  #V_g 2
...
#step_size_BZ = -2e-3 
step_size_BY = 1e-3 
#BZ_vector = arange(0e-3,1000e-3+step_size_BZ,step_size_BZ) #T  # Those two vectors need to be the same left
BY_vector = arange(0e-3,2000e-3+step_size_BY,step_size_BY) #T  #
...
new_mat = np.zeros((len(v1_vec), len(BY_vector))) # Creating empty matrix for storing all data   - ADD THIS LINE FOR MATRIX FILE SAVING, PUT APPROPRIATE VECTOR NAMES
...
try:
    for i,v3 in enumerate(BY_vector):  # CHANGE THIS LINE FOR MATRIX FILE SAVING
