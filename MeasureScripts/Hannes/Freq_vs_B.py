...
v1_vec = arange(0,200.1,0.1)   #V_g
#v2_vec = arange(653,658.1,0.1)  #V_sd 

freq_vec = arange(7.1e9,7.4e9,2e6)
...
new_mat = np.zeros((len(freq_vec), len(v1_vec)),dtype=np.float16) # Creating empty matrix for storing all data   - ADD THIS LINE FOR MATRIX FILE SAVING, PUT APPROPRIATE VECTOR NAMES
...
try:
    for i,v1 in enumerate(v1_vec):  # CHANGE THIS LINE FOR MATRIX FILE SAVING
        ...
        for j,freq in enumerate(freq_vec):  # CHANGE THIS LINE FOR MATRIX FILE SAVING
            ...
            data.add_data_point(freq,v1, result)  
