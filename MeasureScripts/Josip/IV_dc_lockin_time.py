...
v1_vec = arange(0,3,1)     #num of repetitions
v2_vec = arange(-50.0,50.0,0.2)  #V_sd 
...
data_refl = qt.Data(name=' IV_vs_time_13-10_LF_lockin') #just renamed

data_dc = qt.Data(name=' IV_vs_time_13-10_current') #added to have current recored as well
...
data_dc.add_coordinate('num trace')
data_dc.add_value('Current [pA]')


data_refl.add_coordinate('V{SD} [mV]')
data_refl.add_coordinate('num trace')
...
try:
    for i, v1 in enumerate(v1_vec):
        ...
        for j,v2 in enumerate(v2_vec):
            ...
            # readout
            result_reflectometry = UHFLI_lib.UHF_measure_demod(Num_of_TC = 3)/(gain*100)*1e12  # Reading the lockin and correcting for M1b gain
