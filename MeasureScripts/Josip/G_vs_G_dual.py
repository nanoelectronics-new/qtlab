...
v1_vec = arange(320.0,480.0,0.5)   #V_g1  outer loop  07-08
v2_vec = arange(80.0,180.0,0.5)   #    #V_g2   inner loop  06-10
...
try:
    for i,v1 in enumerate(v1_vec):  # CHANGE THIS LINE FOR MATRIX FILE SAVING
        ...
        for j,v2 in enumerate(v2_vec):  # CHANGE THIS LINE FOR MATRIX FILE SAVING
            ...
            # readout
            # making intermediate average
            aver_temp1 = np.array([0,0,0])
            aver_temp2 = np.array([0,0,0])
            for z in xrange(2):
                 result1 = dmm03.get_readval()/gain*1e12
                 result2 = dmm24.get_readval()/gain*1e12
                 aver_temp1[z] += result1
                 aver_temp2[z] += result2
            result1 = sum(aver_temp1)/3.0
            result2 = sum(aver_temp2)/3.0
