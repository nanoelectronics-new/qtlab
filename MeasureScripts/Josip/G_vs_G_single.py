...
name = ' GvsG_23-24-G21'
file_name1 = name+"_reflection"
file_name2 = name+"_current"
...
v1_vec = arange(100.0,215.0,1)    #V_g1   outer loop  23-24
v2_vec = arange(-150.0,150.0,1)  #V_g2   inner loop  22-03
...
data2.add_value('Reflection [arb.u.]')
...
plot2d1 = qt.Plot2D(data1, name='current2D',autoupdate=False)
plot3d1 = qt.Plot3D(data1, name='current3D', coorddims=(1,0), valdim=2, style='image') #flipped coordims that it plots correctly

plot2d2 = qt.Plot2D(data2, name='Reflection2D',autoupdate=False)
plot3d2 = qt.Plot3D(data2, name='Reflection3D', coorddims=(1,0), valdim=2, style='image') #flipped coordims that it plots correctly
...
try:
    for i,v1 in enumerate(v1_vec):  # CHANGE THIS LINE FOR MATRIX FILE SAVING
        ...
        for j,v2 in enumerate(v2_vec):  # CHANGE THIS LINE FOR MATRIX FILE SAVING
            ...
            # readout
            result1 = dmm24.get_readval()/gain*1e12
            result2 = UHFLI_lib.UHF_measure_demod(Num_of_TC = 3)
