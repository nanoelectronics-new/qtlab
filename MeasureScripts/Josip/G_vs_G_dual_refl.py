...
file_name_refl = ' GvsG_23-24-G21_G04_refl'
...
v1_vec = arange(300.0,550.0,0.33)    #V_g1  outer loop  07-08
v2_vec = arange(20.0,180.0,0.33)  #V_g2   inner loop  06-10
...
data_refl = qt.Data(name=file_name_refl)
...
data_temp_refl = np.zeros((len(v2_vec)))
new_mat_refl = np.zeros((len(v2_vec)))
...
data_refl.add_coordinate('V_{G2} [mV]')  #y inner
data_refl.add_coordinate('V_{G1} [mV]')   #x outer
...
plot2d_refl = qt.Plot2D(data_refl, name='2D_23-24_refl',autoupdate=False)
plot3d_refl = qt.Plot3D(data_refl, name='GvsG_23-24_refl', coorddims=(1,0), valdim=2, style='image') #flipped coordims that it plots correctly
...
try:
    for i,v1 in enumerate(v1_vec):  # CHANGE THIS LINE FOR MATRIX FILE SAVING
        ...
        for j,v2 in enumerate(v2_vec):  # CHANGE THIS LINE FOR MATRIX FILE SAVING
            ...
            result_refl = UHFLI_lib.UHF_measure_demod(Num_of_TC = 3)


            # Save to the matrix
            data_temp1[j] = result1
            data_temp2[j] = result2
            data_temp_refl[j] = result_refl

            data1.add_data_point(v2, v1, result1) 
            data2.add_data_point(v2, v1, result2) 
            data_refl.add_data_point(v2, v1, result_refl) 
            qt.msleep(0.005)
        ...
        new_mat_refl = np.column_stack((new_mat_refl, data_temp_refl))
        if not(i):
            new_mat1 = new_mat1[:,1:]
            new_mat2 = new_mat2[:,1:]
            new_mat_refl = new_mat_refl[:,1:]
