...
v1_vec = arange(-50e6,53e6,3e6)  #Frequency offset in Hz
f_center = 5.804e9  # Center frequency in Hz
tau_vector_repetitions = 40
power = 0.5

raw_input("Warning: - check if the sequence and the measurement script have the same number of steps!\nPress enter to continue...")

taus = arange(0.006,0.150,0.001)     # vector of tau values for the Ramsey experiment
...
data.add_coordinate('5.804 GHz + Frequency offset [Hz]')
...
try:
    for i,v1 in enumerate(v1_vec): 
        ...
        v111 = np.linspace(v1,v1,len(taus)) # Vector with repeating value of v1 for the length of the tau vector - for add_data_point command
        data.add_data_point(taus*1e3,v111,tau_vector)  
        data.new_block()
        stop = time()
