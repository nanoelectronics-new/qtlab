...
B_total = arange(100e-3,300e-3,20e-3) # in T

# Pulsed EDSR line coeffs (B in T, f in GHz)
a = 42.133
b = 0.750
...
try:
    for z,B in enumerate(B_total):
        theta = 0.0 # in degrees
        By = B*np.cos(np.deg2rad(theta)) + 20e-3   # First set the field a bit more, so that the final field is set in the sweep down manner
        #Bz = B*np.sin(np.deg2rad(theta))
        
        
        
        
        
        v1_vec = arange(-20e6,24e6,4e6)  #Frequency offset in Hz
        f_center = (a*B + b)*1e9   # Center frequency in Hz
        ...
        By = B*np.cos(np.deg2rad(theta)) # Set the final field
        magnetY.set_field(By)   # Set the By field first               
        while math.fabs(By - magnetY.get_field_get()) > 0.0001:  # Wait until the By field is set
            qt.msleep(0.050)
