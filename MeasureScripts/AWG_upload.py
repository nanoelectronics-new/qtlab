...
if not(Automatic_sequence_generation):  # If user wants manual sequence generation (Automatic_sequence_generation = False)
    ...
    A1 = np.array([0.0,-4.5,0.0,-4.5,0.0])*31.25 # Initial amplitudes, 31.25 is factor of attenuation
    delta_A1 = -2.8*31.25/(Seq_length/1.0) 
    #delta_A1 = (1+1.0/Seq_length)*delta_A1
    A2 = np.array([0.0,4.5,0.0,4.5,0.0])*31.25*0.62 # Initial amplitudes, 31.25 is factor of attenuation, 1.2 is factor for diagonal pulsing
