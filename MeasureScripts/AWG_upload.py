...
if not(Automatic_sequence_generation):  # If user wants manual sequence generation (Automatic_sequence_generation = False)
    ...
    for i in xrange(Seq_length):   # Creating waveforms for all sequence elements
        ...
        p2 = Wav.Waveform(waveform_name = 'WAV2elem%d'%(i+1), AWG_clock = AWG_clock, TimeUnits = 'us' , AmpUnits = 'mV') # helper waveform for creationg the offset for the IQ signal                                                                                                                 # because sync element is first 
        ...
        #cos_len = pul.rescaleLength(p.CH1,0.025) # length of the cosine part
        ...
        #cos_start = pul.rescaleLength(p.CH1,0.0125) - 1
