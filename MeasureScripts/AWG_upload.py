...
num_of_read_int = 1    # Number of read level intervals
...
if not(Automatic_sequence_generation):  # If user wants manual sequence generation (Automatic_sequence_generation = False)
    ...
    A1 = np.array([0.0,-10.0,-4.32,0.0,0.0])*31.25 # Initial amplitudes, 31.25 is factor of attenuation
    delta_A1 = (-10.0*31.25*num_of_read_int)/((Seq_length-num_of_read_int)/1.0)
    #delta_A1 = (1+1.0/Seq_length)*delta_A1
    A2 = np.array([0.0,10.0,4.32,0.0,0.0])*31.25*0.6 # Initial amplitudes, 31.25 is factor of attenuation, 1.2 is factor for diagonal pulsing
    delta_A2 = (10.0*31.25*0.6*num_of_read_int)/((Seq_length-num_of_read_int)/1.0) 
    ...
    for i in xrange(Seq_length):   # Creating waveforms for all sequence elements
        ...
        #else:
        p.setValuesCH1([100, A1[0]],[3500, A1[1]], [500,A1[2]], [500,A1[3]],[100,A1[4]]) # Setting waveform shape for one wavefrom object p in sequence seq for AWG channel 1 - [Time1,Amp1],[Time2,Amp2]...  Time in TimeUnits and Amp in AmpUnits
        ...
        #else:
        p.setValuesCH2([100, A2[0]],[3500, A2[1]], [500,A2[2]], [500,A2[3]],[100,A2[4]]) # Setting waveform shape for one wavefrom object p in sequence seq for AWG channel 1 - [Time1,Amp1],[Time2,Amp2]...  Time in TimeUnits and Amp in AmpUnits
