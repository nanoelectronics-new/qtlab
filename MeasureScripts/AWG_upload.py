...
t_sync = 1000              # Duration of synchronization element in sequence in "TimeUnits"
...
if not(Automatic_sequence_generation):  # If user wants manual sequence generation (Automatic_sequence_generation = False)
    ...
    A1 = np.array([0.0,-8.0,-8.0,0.0,0.0])*20.40 # Initial amplitudes, 31.25 is factor of attenuation  //sensor
    delta_A1 = -(8.0*20.40)/(Seq_length/1.0) 
    delta_A1 = (1.0+1.0/Seq_length)*delta_A1

    A2 = np.array([0.0,-8.0,-8.0,0.0,0.0])*(-0.63)*20.40 # Initial amplitudes, 31.25 is factor of attenuation, 1.2 is factor for diagonal pulsing  //dot
    delta_A2 = (8.0*20.40*0.63)/(Seq_length/1.0)  
    delta_A2 = (1.0+1.0/Seq_length)*delta_A2
