...
AWGMax_amp = 1          # In Volts!!! Maximum needed amplitude on all channels for your particular experiment (noise reduction) - need to be set at the beginning
...
if not(Automatic_sequence_generation):  # If user wants manual sequence generation (Automatic_sequence_generation = False)
    ...
    for i in xrange(Seq_length):   # Creating waveforms for all sequence elements
        ...
        p.setValuesCH1([100, 50],[100, 0]) # Setting waveform shape for one wavefrom object p in sequence seq for AWG channel 1 - [Time1,Amp1],[Time2,Amp2]...  Time in TimeUnits and Amp in AmpUnits
        p.setMarkersCH1([0,1],[0,1])  # Setting marker just in the first wavefrom of the sequence (further is zero)
