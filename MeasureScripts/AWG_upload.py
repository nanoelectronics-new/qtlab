...
Seq_length = 5     # Sequence length (number of periods - waveforms)
...
if not(Automatic_sequence_generation):  # If user wants manual sequence generation (Automatic_sequence_generation = False)
    ...
    A1 = np.array([0.0,-8.0,-3.1,0.0,0.0])*31.25 # Initial amplitudes, 31.25 is factor of attenuation
    delta_A1 = -8.0*31.25/(Seq_length/1.0) 
    #delta_A1 = (1+1.0/Seq_length)*delta_A1
    A2 = np.array([0.0,8.0,3.1,0.0,0.0])*31.25*1.1 # Initial amplitudes, 31.25 is factor of attenuation, 1.2 is factor for diagonal pulsing
    delta_A2 = (8.0*31.25*1.1)/(Seq_length/1.0)  
